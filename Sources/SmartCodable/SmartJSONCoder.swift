...
// JSON Encoder
//===----------------------------------------------------------------------===//
/// `SmartJSONEncoder` facilitates the encoding of `Encodable` values into JSON.
// NOTE: older overlays had Foundation.SmartJSONEncoder as the ObjC name.
// The two must coexist, so it was renamed. The old name must not be
// used in the new runtime. _TtC10Foundation13__SmartJSONEncoder is the
// mangled name for Foundation.__SmartJSONEncoder.
open class SmartJSONEncoder {
    ...
    open func encode<T : Encodable>(_ value: T) throws -> Data {
        let encoder = __SmartJSONEncoder(options: self.options)
...
private struct _JSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    private let encoder: __SmartJSONEncoder
    ...
    init(referencing encoder: __SmartJSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
...
private struct _JSONUnkeyedEncodingContainer : UnkeyedEncodingContainer {
    ...
    // MARK: - Initialization
    /// Initializes `self` with the given references.
    init(referencing encoder: __SmartJSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
...
extension __SmartJSONEncoder : SingleValueEncodingContainer {
...
private extension __SmartJSONEncoder {
    ...
    func box_(_ value: Encodable) throws -> NSObject? {
        ...
        // The value should request a container from the __SmartJSONEncoder.
        let depth = self.storage.count
        do {
...
/// __JSONReferencingEncoder is a special subclass of __SmartJSONEncoder which has its own storage, but references the contents of a different encoder.
/// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
// NOTE: older overlays called this class _JSONReferencingEncoder.
// The two must coexist without a conflicting ObjC class name, so it
// was renamed. The old name must not be used in the new runtime.
private class __JSONReferencingEncoder : __SmartJSONEncoder {
    ...
    let encoder: __SmartJSONEncoder
    ...
    init(referencing encoder: __SmartJSONEncoder, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
    ...
    init(referencing encoder: __SmartJSONEncoder, key: CodingKey, convertedKey: __shared CodingKey, wrapping dictionary: NSMutableDictionary) {
        self.encoder = encoder
...
// JSON Decoder
//===----------------------------------------------------------------------===//
/// `SmartJSONDecoder` facilitates the decoding of JSON into semantic `SmartJSONDecoder` types.
// NOTE: older overlays had Foundation.SmartJSONDecoder as the ObjC name.
// The two must coexist, so it was renamed. The old name must not be
// used in the new runtime. _TtC10Foundation13__SmartJSONDecoder is the
// mangled name for Foundation.__SmartJSONDecoder.
open class SmartJSONDecoder {
    // MARK: Options
    ...
    open func decode<T : Decodable>(_ type: T.Type, from data: Data) throws -> T {
        ...
        let decoder = __SmartJSONDecoder(referencing: topLevel, options: self.options)
...
private class __SmartJSONDecoder : Decoder {
    // MARK: Properties
    /// The decoder's storage.
    var storage: _JSONDecodingStorage
    
    var isSmartDecodable: Bool = false
...
private struct _JSONKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    ...
    private let decoder: __SmartJSONDecoder
    ...
    init(referencing decoder: __SmartJSONDecoder, wrapping container: [String : Any]) {
        ...
        case .convertFromSnakeCase:
            ...
            self.container = Dictionary(container.map {
                key, value in (SmartJSONDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
    ...
    private func _errorDescription(of key: CodingKey) -> String {
        ...
        case .convertFromSnakeCase:
            ...
            let converted = SmartJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(original)
            let roundtrip = SmartJSONDecoder.KeyDecodingStrategy._convertFromSnakeCase(converted)
    ...
    private func _superDecoder(forKey key: __owned CodingKey) throws -> Decoder {
        ...
        return __SmartJSONDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
...
private struct _JSONUnkeyedDecodingContainer : UnkeyedDecodingContainer {
    ...
    // MARK: - Initialization
    /// Initializes `self` by referencing the given decoder and container.
    init(referencing decoder: __SmartJSONDecoder, wrapping container: [Any]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
...
extension __SmartJSONDecoder : SingleValueDecodingContainer {
...
private extension __SmartJSONDecoder {
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String {
                if stringValue == "true"  {
                    ...
                    return Int(0)
                } else if let number = Int(stringValue) {
                    return number
                }
            }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return Int8(0)
                } else if let number = Int8(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return Int16(0)
                } else if let number = Int16(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return Int32(0)
                } else if let number = Int32(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return Int64(0)
                } else if let number = Int64(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return UInt(0)
                } else if let number = UInt(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return UInt8(0)
                } else if let number = UInt8(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return UInt16(0)
                } else if let number = UInt16(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return UInt32(0)
                } else if let number = UInt32(stringValue) {
                    return number
                }
            }
        }
        ...
        if self.isSmartDecodable {
            if let stringValue = value as? String{
                if stringValue == "true" {
                    ...
                    return UInt64(0)
                } else if let number = UInt64(stringValue) {
                    return number
                }
            }
        }
    ...
    func unbox(_ value: Any, as type: String.Type) throws -> String? {
        ...
        if self.isSmartDecodable {
            if let bValue = value as? NSNumber {
                return "\(bValue)"
            }
        }
    ...
    func unbox_(_ value: Any, as type: Decodable.Type) throws -> Any? {
        if type == Date.self || type == NSDate.self {
            ...
            self.storage.push(container: value)
            if type is SmartCodable.Type {
                self.isSmartDecodable = true
            } else {
                self.isSmartDecodable = false
            }
            defer { self.storage.popContainer() }
